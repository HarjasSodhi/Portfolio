...
function ExperienceContent(props: propType) {
    return (
        <div className={`h-full w-full ${props.display ? "block" : "hidden"} animate-[fadein_0.5s_linear_0s_forwards]`}>
            <div className="text-[#ccd6f6] text-xl font-bold">{props.el.position} <span className="text-[#64ffda]">@ <a href={props.el.link} target="_blank" className="un">{props.el.company}</a></span></div>
